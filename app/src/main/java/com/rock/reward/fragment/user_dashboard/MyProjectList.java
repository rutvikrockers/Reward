...
import com.rock.reward.adapter.user_dashboard.MyProjectListAdapter;
...
public class MyProjectList extends Fragment {
    ...
    private List<Project> latestProjectsList;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        recyclerView.addOnScrollListener(new EndlessRecyclerViewScrollListener(linearLayoutManager) {
            ...
            public void onLoadMore(int page, int totalItemsCount) {
                getUserProjects(totalItemsCount);
            }
        });
    ...
    private void getUserProjects(final int offset){
     ...
     //  Call<MyProjectsData> call = restInterface.MyProjects(token,userId,String.valueOf(offset));
      /*  call.enqueue(new Callback<MyProjectsData>() {
            @Override
            public void onResponse(Call<MyProjectsData> call, Response<MyProjectsData> response) {
                Gson gson = new Gson();
                try {
                    ...
                    if(offset==0){
                        latestProjectsList = myProjectsData.getProjectsData();
                        adapter = new MyProjectListAdapter(latestProjectsList,getActivity(),MyProjectList.this);
                        recyclerView.setAdapter(adapter);
                        recyclerView.setLayoutManager(linearLayoutManager);
                    }else{
                        int curSize = adapter.getItemCount();
                        latestProjectsList.addAll(myProjectsData.getProjectsData());
                        adapter.notifyItemRangeInserted(curSize, myProjectsData.getProjectsData().size() - 1);
                    }
...
//                    recyclerView.setLayoutManager(linearLayoutManager);
            ...
            @Override
            public void onFailure(Call<MyProjectsData> call, Throwable t) {
                  t.printStackTrace();
            }
        });*/
